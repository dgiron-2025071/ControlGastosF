...
import { PasivosService } from "./services/pasivos.service";
import { Pasivo, PasivoMonthStats } from "./models/pasivo.model";
...
const CATEGORIAS = [
  "Suscripciones", "Tarjetas", "Alimentacion", "Transporte", "Servicios",
  "Vivienda", "Prestamos", "Deudas", "Salud", "Educacion",
  "Entretenimiento", "Vestimenta", "Estetica", "Mantenimiento", "Impuestos",
];

export interface FiltroPasivos {
  texto: string;
  fechaDesde: string;
  fechaHasta: string;
...
const FILTRO_VACIO: FiltroPasivos = {
  texto: "",
  fechaDesde: "",
  fechaHasta: "",
  montoMin: null,
...
@Component({
  ...
  styleUrl: "./pasivos.component.css",
})
export class PasivosComponent implements OnInit, AfterViewInit, OnDestroy {
  private authService = inject(AuthService);
  private sessionService = inject(SessionService);
  private toastService = inject(ToastService);
  private router = inject(Router);
  private store = inject(FinanceStoreService);
  private pasivosService = inject(PasivosService);
  ...
  items = signal<Pasivo[]>([]);
  stats = signal<PasivoMonthStats | null>(null);
  loading = signal(true);
  saving = signal(false);
  editMode = signal(false);
  showFilters = signal(false);
  showCreateModal = signal(false);
  filters = signal<FiltroPasivos>({ ...FILTRO_VACIO });

  editCopies: Record<number, Pasivo> = {};
  archiving = new Set<number>();
  ...
  createForm = this.fb.group({
    ...
    tasaInteres: [0],
    fechaVencimiento: [this.fechaDelMesSeleccionado(), Validators.required],
    descripcion: [""],
  });
    ...
    return this.items().filter((item) => {
      if (texto) {
        const match =
          ...
          (item.fechaVencimiento ?? "").includes(texto);
        if (!match) return false;
      }
      if (f.fechaDesde && (item.fechaVencimiento ?? "") < f.fechaDesde) return false;
      if (f.fechaHasta && (item.fechaVencimiento ?? "") > f.fechaHasta) return false;
  ...
  showsArchiving = computed(() => this.archiving.size > 0);
  ...
  loadMonth(): void {
    ...
    this.pasivosService.listMonth(this.currentYear, this.currentMonth).subscribe({
      next: (data) => {
        this.items.set(data.items);
        this.stats.set(data.stats);
        this.cleanupEditCopies();
        this.archiving.clear();
      ...
      error: () => {
        ...
        this.stats.set(null);
        this.loading.set(false);
        this.toastService.error("No se pudieron cargar los pasivos.");
      },
    });
  }
  ...
  setFilter(key: keyof FiltroPasivos, value: string | number | null): void {
  ...
  saveEdit(id: number): void {
    ...
    this.pasivosService
      .update(id, {
        ...
        categoria: copy.categoria || "Suscripciones",
        descripcion: copy.descripcion ?? "",
        monto,
        tasaInteres: Number(copy.tasaInteres) || 0,
        fechaVencimiento: copy.fechaVencimiento || null,
      })
      .subscribe({
        next: () => {
          ...
          this.toastService.success("Pasivo actualizado correctamente.");
        },
        error: () => {
          this.saving.set(false);
          this.toastService.error("No se pudo actualizar el pasivo.");
        },
      });
  }
  ...
  deleteItem(id: number, nombre: string): void {
    const confirmacion = window.confirm(
      `¿Deseas eliminar el pasivo "${nombre}"?`
    );
    if (!confirmacion) return;

    this.archiving.add(id);
    this.pasivosService.remove(id).subscribe({
      next: () => {
        this.archiving.delete(id);
        this.store.notifyDataChanged();
        this.toastService.success("Pasivo eliminado correctamente.");
      },
      error: () => {
        this.archiving.delete(id);
        this.toastService.error("No se pudo eliminar el pasivo.");
      },
    });
  }

  markPaid(id: number, nombre: string): void {
    const confirmacion = window.confirm(
    ...
    this.archiving.add(id);
    this.pasivosService.markPaid(id).subscribe({
      next: () => {
        this.archiving.delete(id);
        this.store.notifyDataChanged();
        this.toastService.success("Pasivo marcado como pagado.");
      },
      error: (err) => {
        this.archiving.delete(id);
  ...
  /** Fecha máxima permitida: un pasivo no puede tener fecha futura.
   *  Calculada en hora local del navegador (YYYY-MM-DD). */
  hoyFecha(): string {
  ...
  openCreateModal(): void {
    this.createForm.reset({
      ...
      monto: null,
      tasaInteres: 0,
      fechaVencimiento: this.fechaDelMesSeleccionado(),
      descripcion: "",
    });
  ...
  submitCreate(): void {
    if (this.createForm.invalid) {
      this.toastService.error("Complete los campos obligatorios (nombre, monto y fecha).");
      return;
    }

    const raw = this.createForm.value;
    const monto = Number(raw.monto ?? 0);
    const fecha = String(raw.fechaVencimiento ?? this.fechaDelMesSeleccionado());
    ...
    this.pasivosService
      .create({
        ...
        empresa: raw.empresa ?? "",
        categoria: raw.categoria ?? "Suscripciones",
      ...
      .subscribe({
        next: () => {
          ...
          this.toastService.success("Pasivo registrado correctamente.");
        },
        error: () => {
          this.saving.set(false);
          this.toastService.error("No se pudo registrar el pasivo.");
        },
      });
  }
